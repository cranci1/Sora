...
struct SettingsViewData: View {
        @State private
        var showEraseAppDataAlert = false
        @State private
        var showRemoveDocumentsAlert = false
        @State private
        ...
        @State private
        var movPkgSize: Int64 = 0
        @State private
        var showRemoveMovPkgAlert = false

        @State private
        var isMetadataCachingEnabled: Bool = true
        @State private
        var isImageCachingEnabled: Bool = true
        @State private
        var isMemoryOnlyMode: Bool = false
        ...
        var showAlert = false
        @State private
        var activeAlert: ActiveAlert = .eraseData
        ...
        var body: some View {
                ...
                VStack(spacing: 24) {
                        SettingsSection(
                            ...
                            SettingsToggleRow(
                                ...
                                .onChange( of: isMetadataCachingEnabled) {
                                    newValue in
                            ...
                            SettingsToggleRow(
                                ...
                                .onChange( of: isImageCachingEnabled) {
                                    newValue in
            ...
            func calculateCacheSize() {
                ...
                DispatchQueue.global(qos: .background).async {
                    ...
                    KingfisherCacheManager.shared.calculateCacheSize {
                        imageSize in
            ...
            private func calculateDirectorySize(
                for url: URL) - > Int64 {
                let fileManager = FileManager.default
                var totalSize: Int64 = 0
                ...
                do {
                    ...
                    for url in contents {
                        ...
                        if resourceValues.isDirectory == true {
                            totalSize += calculateDirectorySize(
                                for: url)
                        } else {
            ...
            private func updateSizes() {
                if let cacheURL = FileManager.default.urls(
                    for: .cachesDirectory, in: .userDomainMask).first {
                    cacheSize = calculateDirectorySize(
                        for: cacheURL)
                }

                if let documentsURL = FileManager.default.urls(
                    for: .documentDirectory, in: .userDomainMask).first {
                    documentsSize = calculateDirectorySize(
                        for: documentsURL)
