...
// Extension to detect scrolling so ContentView hides
extension View {
    func trackScroll() -> some View {
                                ...
                                VStack(spacing: 0) {
                                    ...
                                    LinearGradient(
                                        gradient: Gradient(colors: [
                                            settings.selectedAppearance == .dark ? Color.black.opacity(1) : Color.white.opacity(1),
                                            settings.selectedAppearance == .dark ? Color.black.opacity(0) : Color.white.opacity(0)
                                        ]),
                                        startPoint: .top,
                                        endPoint: .bottom
                                    )
...
private func fetchBannerImage() {
    guard let itemID = itemID else {
        Logger.shared.log("AniList ID is not available", type: "Error")
        isLoading = false
        return
    }
    
    guard let url = URL(string: "https://api.ani.zip/mappings?anilist_id=\(itemID)") else {
        Logger.shared.log("Invalid URL for fetching banner image", type: "Error")
        isLoading = false
        return
    }
    ...
    URLSession.custom.dataTask(with: url) { data, _, error in
        if let error = error {
            Logger.shared.log("Failed to fetch banner image: \(error)", type: "Error")
            DispatchQueue.main.async {
                self.isLoading = false
            }
            return
        ...
        guard let data = data else {
            Logger.shared.log("No data received while fetching banner image", type: "Error")
            DispatchQueue.main.async {
                self.isLoading = false
            }
            return
        ...
        do {
            ...
            guard let json = jsonObject as? [String: Any],
                ...
                Logger.shared.log("Invalid response format while fetching banner image", type: "Error")
                DispatchQueue.main.async {
                    self.isLoading = false
                }
                return
            ...
            if let banner = images.first(where: { ($0["coverType"] as? String) == "Fanart" }),
               let url = banner["url"] as? String {
                bannerUrl = url
            }
            ...
            Logger.shared.log("Error parsing JSON while fetching banner image: \(error)", type: "Error")
            DispatchQueue.main.async {
                self.isLoading = false
            }
        }
